...
static Args *args_new(const gchar *param)
{
	Args *args;
	gchar **kv_array;
	GError *error = NULL;
	...
	for (size_t i = 0; kv_array[i] != NULL; i++)
	{
		...
		{
			g_warning("Given parameter '%s' is not key=value form.", kv_array[i]);
			g_strfreev(kv);
			continue;
		}
		...
		if (g_str_equal(k, "system_prompt_file") && (args->system_prompt == NULL))
		{
			if (!g_file_get_contents(v, &args->system_prompt, NULL, &error))
			{
				g_warning("Failed to read file '%s'. This cause no system prompt.\n%s", v, error->message);
...
on_error:
	g_warning(
		"Required parameter(s) is/are not provided.\n"
		"You need:\n"
...
static gboolean prompt(LMModule *module, GString *str, GError **error)
{
	LMRealModule *rmodule = (LMRealModule*)module;
	size_t token_len = 0;
	int64_t *tokens = NULL;
	...
	if (tokens == NULL)
	{
		g_set_error(error, RWKV_MODULE_ERROR, -1, "Failed to tokenize given string '%s'", str->str);
		return FALSE;
	}
	...
	if (!mm_model_run(rmodule->model, rmodule->input, rmodule->prompt_output, error))
		goto on_error;
...
G_MODULE_EXPORT LMModule *init(LMModuleParam *param, GError **error) {
  ...
  if (args->state_file)
  {
	  ...
	  {
		  MMValue *v = module->value_array->pdata[k];
		  if (g_strcmp0(v->input_name, "x") && g_strcmp0(v->output_name, "y"))
  ...
  {
	  GString *system_prompt = chat_template("System", args->system_prompt, NULL);
