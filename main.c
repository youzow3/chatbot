...
args_new (int *argc, char ***argv, GError **error)
{
  ...
  entries[ARGS_LM_MODULE_PARAM] = (GOptionEntry){ "lm_module_param",
                                                  ...
                                                  "param" };
  entries[ARGS_SHOW_THINKING] = (GOptionEntry) {"show_thinking", 0, G_OPTION_FLAG_NONE, G_OPTION_ARG_NONE, &args->show_thinking, "Show thinking when LLM thinks", NULL};
...
Module *
module_new (char *file_name, void *param, GError **error)
{
  Module *module;
  ...
  while (TRUE)
    {
      ...
      chat_template
          = lm->chat_template ("User", user_input->str, "Assistant", NULL);
      printf ("\nAssistant: ");
      fflush (stdout);
      if (thinking && args->show_thinking)
        output = lm->inference_with_think (lm, chat_template, NULL, callback,
                                           callback, NULL, &error);
      else if (thinking)
	output = lm->inference_with_think(lm, chat_template, NULL, NULL, callback, NULL, &error);
      else
        output = lm->inference (lm, chat_template, callback, NULL, &error);
      if (output == NULL)
