 ...
 *
 * Interface for modules that implements language model. All methods of this
 * interface is considered as batch_size == 1 with variable length tokens.
 * Implementer should decide what model and what strategy to sample, and this
 * interface doesn't specify way to generate text. Implementer can use greedy
 * sampling, some sort of random sampling, or tree and broute force.
 */
...
chatbot_language_model_default_init (ChatbotLanguageModelInterface *iface)
{
   ...
   * [method@generate] should emit this signal same count as how many tokens
   * are generated. Emitter should supply generated token's string.
   */
 ...
 *
 * Perform prefill by given text. Returns %TRUE if success, %FALSE on failure.
 *
